...
static std::string next_line(std::string *shader, size_t *pos)
{
...
uint64_t parse_global_flags_to_sfi(std::string *shader)
{
	uint64_t sfi = 0LL;
	std::string line;
...
static uint64_t parse_subshader_feature_info_comment(std::string *shader, size_t *pos, uint64_t flags)
{
	std::string line;
	size_t old_pos = *pos;
	uint32_t i;
...
static bool is_hull_shader(std::string *shader, size_t start_pos) 
{
	std::string line;
...
static bool is_geometry_shader_5(std::string *shader, size_t start_pos) {
	std::string line;
	size_t pos = start_pos;
...
static HRESULT manufacture_shader_binary(const void *pShaderAsm, size_t AsmLength, std::vector<byte> *bytecode)
{
	std::string shader_str((const char*)pShaderAsm, AsmLength);
	std::string line;
	size_t pos = 0;
	bool done = false;
